"""Class with functions that calculate the Area Under Kappa score for binary classification models.

Code originally written by Emil Rijcken and published on https://towardsdatascience.com/auk-a-simple-alternative-to-auc-800e61945be5

To run, use the code in the following way:

    auk_class = AUK(probabilities, labels)
    auk_score = auk_class.calculate_auk()
    kappa_curve = auk_class.kappa_curve()

"""
import numpy as np

class AUK:
    def __init__(self, labels, probabilities, integral='trapezoid'):
        self.probabilities = list(probabilities.flatten())
    ...
    #make list with tpr scores for each threshold
    def tpr_curve(self):
        tpr_list = []
        ...
        for thres in self.probabilities_set:
            ...
            tpr_list.append(tpr)
        return self._add_zero_to_curve(tpr_list)
