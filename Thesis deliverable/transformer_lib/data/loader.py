...
class LogsDataLoader:
    def __init__(self, name, dir_path = "./datasets"):
        ...
        """
        self._dir_path = f"C:/Users/20190337/Downloads/Tracebook_v2 (Projectfolder)/encoded_logs/transformer/{name}/processed"
    ...
    def prepare_data_outcome(self, df, 
        x_word_dict, 
        max_case_length, shuffle=True):
        
        x = df["activities"].values
        y = df["outcome"].values
        if shuffle:
            x, y = utils.shuffle(x, y)
    ...
    def load_data(self, task):
        ...
        full_df = pd.read_csv(f"{self._dir_path}/{task}_full.csv")
        ...
        if task == 'outcome':
            max_case_length = self.get_max_case_length(train_df["activities"].values)
        else:
