...
class LogsDataProcessor:
    ...
    def _load_df(self, sort_temporally = False):
        ...
        df.columns = ["case:concept:name", 
            "concept:name", "time:timestamp", "Label"]
    ...
    def _outcome_helper_func(self, df):
        case_id, case_name = "case:concept:name", "concept:name"
        processed_df = pd.DataFrame(columns = ["case_id", "activities", "outcome"])
        idx = 0
        unique_cases = df[case_id].unique()
        for _, case in enumerate(unique_cases):
            act = df[df[case_id] == case][case_name].to_list()
            for i in range(len(act) - 1):
                activities = np.where(i == 0, act[0], " ".join(act[:i+1]))        
                next_act = act[i+1]
            outcome = int(df[df[case_id] == case]['Label'].mean())
            processed_df.at[idx, "case_id"]  =  case
            processed_df.at[idx, "activities"]  =  activities
            processed_df.at[idx, "outcome"] = outcome
            idx = idx + 1
        return processed_df

    def _process_outcome(self, df, train_list, test_list, full_list):
        df_split = np.array_split(df, self._pool)
        with Pool(processes=self._pool) as pool:
        ...
        full_df.to_csv(f"{self._dir_path}/outcome_full.csv", index = False)
    ...
    def process_logs(self, task, 
        ...
        full_list = df["case:concept:name"].unique()
        if task == 'NEXT_ACTIVITY':
            ...
            self._process_outcome(df, train_list, test_list, full_list)
        else:
